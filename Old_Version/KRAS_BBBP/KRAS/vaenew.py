...
class SmilesEncoder(nn.Module):
    def __init__(self, vocab_size, hidden_size, latent_size, pad_idx, num_layers, dropout, device=None):
        ...
        self.device = torch.device('cuda:1') if device is None else device
...
class SmilesAutoencoder(nn.Module):
    ...
    def policy_gradient(self, dataset, reward_ft,
                        batch_size=5, total_steps=10, learning_rate=1e-4, discount_factor=0.995, buffer_size=10, buffer_batch_size=10,
                        validation_dataset=None, validation_repetition_size=5,
                        display_step=10, validation_step=5, checkpoint_step=10, checkpoint_filepath=None, verbose=1):
        ## Flag of GPU
        use_cuda = torch.cuda.is_available()
    ...
    def fit(self, dataset,
            batch_size=100, total_steps=2, learning_rate=1e-3,
            validation_dataset=None, validation_repetition_size=20,
            checkpoint_step=1, checkpoint_filepath=None, display_step=2,
