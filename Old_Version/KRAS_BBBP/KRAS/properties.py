...
import COMA.bbb_scorer as BBB
import COMA.affnity_scorer as AFF
...
def aff(s):
    if s is None:
        ...
        return AFF.get_affnity_score_mapped(s)
...
def bbb(s):
    if s is None:
        ...
        return BBB.get_bbb_score(s)

def drd2(s):
    return float(0.4 * aff(s) + 0.4 * bbb(s) + 0.2 * qed(s))
